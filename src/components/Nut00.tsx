...
import { Lock, Unlock, Key, RefreshCw, CheckCircle, Eye, EyeOff } from 'lucide-react';
  ...
  const renderOverview = () => (
    <div className="space-y-6">
      <div className="bg-gradient-to-r from-purple-500 to-blue-500 rounded-lg p-8 text-white">
        <h2 className="text-3xl font-bold mb-4">NUT-00: {t('Protocol Foundation')}</h2>
      ...
      <div className="space-y-4">
        {bdhkeSteps.map((step) => (
          <div
            ...
            onClick={() => setSelectedStep(selectedStep === step.num ? null : step.num)}
          >
            <div className="p-4 flex items-start gap-4">
              ...
              <div className="flex-1">
                <div className="flex items-center gap-2 mb-1">
                  <span className="font-semibold text-gray-700">{step.actor}</span>
                  <span className="text-gray-400">→</span>
                  <span className="text-gray-600">{step.action}</span>
                </div>
                <code className="text-sm bg-gray-400 px-2 py-1 rounded">{step.detail}</code>
              </div>
              {selectedStep === step.num ? (
        ...
        <div className="space-y-2">
          {variables.common.map((v, i) => (
            <div key={i} className="flex items-start gap-3">
              <code className="bg-gray-400 px-3 py-1 rounded font-mono text-sm border">{v.symbol}</code>
              <div>
      ...
      <div className="bg-blue-50 rounded-lg p-6">
        ...
        <div className="space-y-2">
          {variables.bob.map((v, i) => (
            <div key={i} className="flex items-start gap-3">
              <code className="bg-gray-400 px-3 py-1 rounded font-mono text-sm border border-blue-200">{v.symbol}</code>
              <div>
                <div className="font-semibold text-gray-600">{v.name}</div>
                <div className="text-sm text-gray-600">{v.desc}</div>
              </div>
      ...
      <div className="bg-purple-50 rounded-lg p-6">
        ...
        <div className="space-y-2">
          {variables.alice.map((v, i) => (
            <div key={i} className="flex items-start gap-3">
              <code className="bg-gray-400 px-3 py-1 rounded font-mono text-sm border border-purple-200">{v.symbol}</code>
              <div>
                <div className="font-semibold text-gray-600">{v.name}</div>
                <div className="text-sm text-gray-600">{v.desc}</div>
              </div>
...
{`{
  ...
  "B_": hex_str
          ...
          <ul className="mt-3 space-y-1 text-sm text-gray-600">
            <li><strong>{t('amount')}:</strong> {t('Value for requested signature')}</li>
            <li><strong>id:</strong> {t('Keyset ID from')}</li>
            <li><strong>B_:</strong> {t('Blinded secret message')}</li>
          </ul>
        </div>
      </div>
      ...
      <div className="bg-white rounded-lg border-2 border-blue-200 overflow-hidden">
        <div className="bg-blue-500 text-white p-4">
          ...
          <p className="text-sm opacity-90">{t('Sent from Bob to Alice after signing')}</p>
        </div>
        <div className="p-4">
          <pre className="bg-gray-50 text-black text-black p-4 rounded text-sm overflow-x-auto">
{`{
  ...
  "C_": hex_str
          ...
          <ul className="mt-3 space-y-1 text-sm text-gray-600">
            <li><strong>{t('amount')}:</strong> {t('Value of the blinded token')}</li>
            <li><strong>id:</strong> {t('Keyset ID of signing keys')}</li>
            <li><strong>C_:</strong> {t('Blind signature on secret B_')}</li>
          </ul>
        </div>
      </div>
      ...
      <div className="bg-white rounded-lg border-2 border-green-200 overflow-hidden">
        <div className="bg-green-500 text-white p-4">
          ...
          <p className="text-sm opacity-90">{t('Used by Alice to spend tokens')}</p>
        </div>
        <div className="p-4">
          <pre className="bg-gray-50 text-black text-black p-4 rounded text-sm overflow-x-auto">
{`{
  ...
  "C": hex_str
          ...
          <ul className="mt-3 space-y-1 text-sm text-gray-600">
            <li><strong>{t('amount')}:</strong> {t('Value of the proof')}</li>
            <li><strong>id:</strong> {t('Keyset ID of signing keys')}</li>
            <li><strong>secret:</strong> {t('The secret message (UTF-8 string)')}</li>
            <li><strong>C:</strong> {t('Unblinded signature on secret')}</li>
          </ul>
        </div>
      </div>
    </div>
  ...
  const renderTokens = () => (
    <div className="space-y-6">
      <h2 className="text-2xl font-bold text-gray-400 mb-4">{t('Token Serialization')}</h2>
      ...
      <div className="bg-gradient-to-r from-blue-500 to-purple-500 text-white rounded-lg p-6">
        ...
        <p className="mt-3 text-sm">
          {t('URI scheme for clickable tokens')}: <code className="bg-gray-400/20 px-2 py-1 rounded">cashu:cashuA...</code>
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="bg-white rounded-lg border-2 border-blue-200 p-6">
          <div className="flex items-center gap-2 mb-3">
            <span className="bg-blue-500 text-white px-2 py-1 rounded font-mono text-sm">V3</span>
            <h3 className="font-bold">{t('Legacy Format')}</h3>
          </div>
          <ul className="space-y-2 text-sm text-gray-600">
            <li>• {t('Version flag')}: <code>A</code></li>
        ...
        <div className="bg-white rounded-lg border-2 border-purple-200 p-6">
          <div className="flex items-center gap-2 mb-3">
            <span className="bg-purple-500 text-white px-2 py-1 rounded font-mono text-sm">V4</span>
            <h3 className="font-bold">Current Format</h3>
          </div>
          <ul className="space-y-2 text-sm text-gray-600">
            <li>• {t('Version flag')}: <code>B</code></li>
      ...
      <div className="bg-white rounded-lg border-2 border-gray-200 p-6">
        <h3 className="font-bold text-lg text-black mb-3">{t('V4 Token Structure')}</h3>
  ...
  "t": [         // token array
    {
      ...
      <div className="bg-blue-50 border-l-4 border-blue-500 p-6 rounded">
        <h3 className="font-bold mb-2 text-black">{t('Binary Format (NFC)')}</h3>
        <p className="text-gray-700 mb-2">
          {t('For NFC transmission')}:
        </p>
  ...
  return (
    <div className="min-h-screen bg-linear-to-br from-purple-900 via-blue-900 to-indigo-900 text-white">
      <div className="w-full max-w-7xl mx-auto px-4 py-8 md:px-6 md:py-12">
        ...
        {/* Content */}
        <div className="rounded-lg shadow-xl p-6 md:p-8">
        ...
        <div className="text-center mt-8 text-gray-600 text-sm">
          <p>{t('Part of the Cashu Protocol Specification')}</p>
          <p className="mt-2">
            Learn more at{' '}
            <a href="https://docs.cashu.space" className="text-blue-500 hover:underline">
              docs.cashu.space
            </a>
          </p>
        </div>
